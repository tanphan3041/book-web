...
function getDescriptor(filename, options, createIfNotFound = true) {
  ...
  if (createIfNotFound) {
    const { descriptor, errors } = createDescriptor(
      filename,
      import_node_fs2.default.readFileSync(filename, "utf-8"),
      options
    );
    if (errors.length) {
      throw errors[0];
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (options.devServer) {
    if (filename.startsWith(options.root)) {
      ...
      assetUrlOptions = {
        base: ((_c = (_b = options.devServer.config.server) == null ? void 0 : _b.origin) != null ? _c : "") + devBase + slash(import_node_path2.default.relative(options.root, import_node_path2.default.dirname(filename)))
      };
    }
...
async function transformMain(code, filename, options, pluginContext, ssr, asCustomElement) {
  ...
  const lang = ((_c = descriptor.scriptSetup) == null ? void 0 : _c.lang) || ((_d = descriptor.script) == null ? void 0 : _d.lang);
  if (lang && /tsx?$/.test(lang) && !((_e = descriptor.script) == null ? void 0 : _e.src)) {
